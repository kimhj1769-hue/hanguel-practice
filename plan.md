...
## v1.3 — 게임성 강화
- [x] **게임 메뉴 추가** (메인 화면 🎮 카드 → 게임 선택 화면)
  - [x] 모드 A: 타이핑 벽돌깨기 — 내려오는 벽돌의 글자를 타이핑하면 펭귄이 눈덩이 발사·격파. 위험선 도달 시 생명 차감, 시간 경과로 난이도 상승
  - [x] 모드 B: 클래식 벽돌깨기(알카노이드) — 마우스/←→로 막대 이동, 스페이스·클릭 발사, 자모 벽돌 격파, 레벨 클리어 시 가속
  - [x] Canvas 기반 엔진(requestAnimationFrame), 파티클 이펙트, 생명/점수 HUD, 게임오버 오버레이
